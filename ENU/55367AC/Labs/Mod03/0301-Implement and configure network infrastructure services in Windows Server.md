...
1. In **Microsoft Edge**, select **Windows Admin Center**.
   ...
   > **Note:** It may take a minute or two for the Windows Admin Center services to start.
...
1. On the **Tools** pane, select **Overview**, and then on the details pane take note of the message to reboot LON-SVR1. 
...
1. On the **Tools** pane, select **DHCP**, and then on the details pane, select **Install**. 
     ...
     > If DHCP is not available in the **Tools** pane for **LON-SVR1**, close **Microsoft Edge** and sign in to **Windows Admin Center** again.
...
1. On **LON-ADM1**, in **Windows Admin Center**, while connected to **LON-SVR1**, in the **Tools** pane, select **DHCP**, and then select **New scope**.
...
1. In the **Servers** details pane, right-click or access the context menu for **LON-SVR1**, and then select **DHCP Manager**.
1. In DHCP window, expand **LON-SVR1.Contoso.com**, expand **IPv4**, expand **Scope [10.100.150.0] TreyClients**, and then select **Scope Options**.
...
1. In the **Servers** details pane, right-click or access the context menu for **LON-DC1**, and then select **DHCP Manager**.
...
1. On the **Specify the partner server to use for failover** screen, in the **Partner Server** box, enter **LON-SVR1**, select the **Reuse existing failover relationships configured with this server (if any exist)** check box, and then select **Next**.
...
1. Under **LON-SVR1.Contoso.com**, select **IPv4**, and then verify that both scopes are listed. If necessary, refresh the console to view both scopes.
...
1. In the **Install Roles and Features** dialog box, select **Yes**.
   ...
   > Wait until a notification appears indicating that the DNS role is installed. If necessary, select the **Notifications** icon to verify the current status.

1. In **Windows Admin Center**, select the **Server Manager** drop-down menu, and then select **All connections**.

1. Select **LON-SVR1.contoso.com**.

1. In the **Tools** pane, select **DNS**, and then on the details pane, select **Install**. 

   > Note: If **DNS** is not available in the **Tools** pane for **LON-SVR1**, close **Microsoft Edge** and sign in to **Windows Admin Center** again.
...
1. On **LON-ADM1**, select **Start**, and then select **Server Manager**.
1. In **Server Manager**, select **All Servers**.
1. In the **Servers** details pane, right-click or access the context menu for **LON-SVR1**, and then select **DNS Manager**.
...
1. In the **IP addresses of the master servers** box, enter **172.16.0.10**, and then select **Enter** and then select **OK**.
...
1. At the **Windows PowerShell** prompt, enter **`ipconfig`**, and then select **Enter**. 
    ...
    > Note that the Ethernet adapter has an IP address that is part of the HeadOfficeSubnet (172.16.0.0) configured in the policy.

1. To test the DNS policy, at the Windows PowerShell prompt enter the following command and then press Enter:
