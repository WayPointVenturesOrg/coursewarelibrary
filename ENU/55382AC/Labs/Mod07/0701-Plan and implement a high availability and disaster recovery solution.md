...
Contoso Ltd. is looking to assess and configure new high availability features and technologies available by using Windows Server 2022. As the system administrator, you have been tasked with performing that assessment and implementation. 
...
Contoso Ltd. has its headquarters in New York. It is reviewing its current disaster recovery strategy after a recent fire in a remote office in London resulted in the loss of some data. It was also decided to review current strategies around high availability at the company. Contoso is considering an upgrade to Windows Server 2022 and wants to determine if there are any features that it can leverage. Budgets are also under pressure and management is looking to see if there are any cost savings that can be can realized to help offset the expenditure to replace existing legacy storage currently being used with a Hyper-V cluster. Contoso has the following business requirements:
...
- The finance team requires less than 1 min downtime for their RTO and zero data loss as their RPO on their customer facing transactions.
...
After completing this exercise, you will have successfully designed the appropriate high availability and disaster recovery solution that will meet your organization’s requirements.

## Exercise 2: Implement storage migration

### Scenario

To upgrade the storage for the new host servers, you need to be able to to move existing virtual machine storage between Hyper-V hosts without service downtime. You plan to test Hyper-V storage migration and verify that it will meet your requirements. 
...
2. On the task bar, select **Hyper-V Manager**.

   > The Hyper-V Manager console opens.

3. In Hyper-V Manager, in the navigation pane, select **LON-SVR2**.

   > When you select **LON-SVR2**, the details and Actions panes display to manage virtual machines available on this host.
...
13. On the **Completing Move Wizard** page, select **Finish**. 
    ...
    > **Note:** A message box that states **Performing the move** is displayed. While the move is completing, connect to **LON-SVR5** and verify that the VM is still accessible during the move. Wait until the move is complete.
...
You need to evaluate the Hyper-V Replica feature used for replicating virtual machines between hosts. You want to be able to mount a copy of a virtual machine to another host if the active copy or host fails. 

The main tasks for this exercise are as follows:

1. Enable replication on both host machines.
2. Configure replication for LON-SVR5 VM.
...
2. On **LON-SVR3**, on the task bar, select **Hyper-V Manager**.
...
8. Expand **This PC**, select to expand **Local Disk (C)**, and then select **New folder**. For the folder name, type **VMReplica**, and then press Enter. 
...
4. On **LON-SVR2**, in Hyper-V Manager, right-click or access the context menu for **LON-SVR5** and then select **Shut Down**.
   ...
   > Wait for **LON-SVR5** to shut down. In order to perform a planned failover, the VM must not be running.
...
After completing this exercise, you will have configured and validated Hyper-V Replica.
