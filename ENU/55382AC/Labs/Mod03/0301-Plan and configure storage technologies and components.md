...
You are a Storage Administrator for Contoso, Ltd. and part of your job is to ensure that your data storage systems meet both short-term and long-term business needs that evolve regularly. 
...
Contoso Ltd. wants to design new storage solutions to support several recent changes. These changes include:
...
- A recently acquired company uses a different IT infrastructure than Contoso. The IT department now needs to manage a mixed environment that includes remote geographical areas in London, New York, and Japan.

- The cost of storage has decreased significantly over recent years.

- The amount of data produced by Contoso business groups has increased even faster.


### Requirements

In general, the new system should be low‑cost, have reasonable performance, and the storage administrators in Contoso and the newly acquired company should be able to manage it easily.
...
- The provisioning of VMWare ESXi virtual machines.
...
As a senior server administrator at Contoso, you are responsible for implementing the new file storage technologies for the organization. After reviewing the requirements, you propose a plan based on answers to the following questions:
...
- Which server role(s) do you plan to use for the provisioning of VMWare ESXi virtual machines?
...
4. Which server role(s) do you plan to use for the provisioning of VMWare ESXi virtual machines?

   > You can use the Server for NFS role to create NFS file shares that VMWare ESXi virtual machines support.
...
7. How do you plan to disable legacy SMB access for existing SMB file shares?
   ...
   > First, you need to locate all the existing file servers that have legacy SMB shares. You can check for legacy shares on the current host by using the following command at a Windows PowerShell prompt:
   >
   > ```
   ...
   >
   > After you have located all of the existing file servers that have legacy SMB shares, you need to disable the SMB access. You can disable the SMB 1 protocol on each server by using the following command:
   >
   > ```
...
6. On the **Select server roles** page, expand **File and Storage Services (2 of 12 Installed)**, expand **File and iSCSI Services (1 of 11 Installed)**, select the **iSCSI Target Server** check box, and then select **Next**.
...
8. On the **Specify target name** page, in the **Name** box, enter **LON‑DC1**, and then select **Next**.

9. On the **Specify access servers** page, select **Add**.

10. In the **Select a method to identify the initiator** dialog box, select **Enter a value for the selected type**, in the **Type** list, select **IP Address**, in the **Value** text box, enter **10.100.100.3**, and then select **OK**.
...
19. On the **Specify iSCSI virtual disk name** page, in the **Name** box, enter **iSCSIDisk2**, and then select **Next**.

20. On the **Specify iSCSI virtual disk size** page, in the **Size** box, enter **5**, ensure that **GB** is selected, and then select **Next**.
...
11. Restart **LON-SVR1**, and then sign in as **Contoso\\Administrator** with the password **Pa55w.rd**.
...
15. In the **iSCSI Initiator Properties** dialog box, on the **Targets** tab, in the **Target** box, enter **10.100.100.2**, and then select **Quick Connect**.

16. In the **Quick Connect** box, select **Done**.

17. In the **iSCSI Initiator Properties** dialog box, to close the dialog box, select **OK**.
...
23. In the **MPIO Properties** dialog box, to close the dialog box, select **OK**.
...
5. In the **Connect To Target** dialog box, select the **Enable multi-path** check box, verify that the **Add this connection to the list of Favorite Targets** check box is selected, and then select **Advanced**.
...
12. In the **Connect To Target** dialog box, select **Enable multi-path**, verify that the **Add this connection to the list of Favorite Targets** check box is selected, and then select **Advanced**.
...
4. Right-click or access the context menu for the iSCSI disk that you brought online, and then select **New Volume**.
...
7. In the **Offline or Uninitialized Disk** dialog box, to initialize the disk as a GPT disk, select **OK**.

8. On the **Specify the size of the volume** page, to accept the default of using the entire disk size for the volume, select **Next**.
...
15. Right-click or access the context menu for the iSCSI disk that you brought online, and then select **New Volume**.
...
19. On the **Specify the size of the volume** page, select **Next** to accept the default of using the entire disk size for the volume.
...
23. On the **Confirm selections** page, to finish creating the volume, select **Create**.

24. After the volume is created, on the **Completion** page, select **Close**.

25. On the taskbar, select **File Explorer**, browse to **This PC**, and then verify that the **SMBShares** and **NFSShares** volumes are displayed.
...
5. On the **Specify share name** page, in the **Share name** box, enter **Data**, and then select **Next**.

6. On the **Configure share settings** page, select the **Enable access-based enumeration** check box, and then select **Next**.
...
10. In the **Basic permissions** area, select the **Modify** check box, and then select **OK**.
...
14. When the creation of the share is complete, select **Close**.
...
4. On the **Specify share name** page, in the **Share name** box, enter **LinuxData**, and then select **Next**.
...
3. In **File Explorer**, in the address bar, enter **\\\\LON-SVR1\\Data**, and then press Enter.

4. Select the **Home** tab, select **New item**, and then select **Text Document**.

5. Enter **NewFile**, and then press Enter to rename the document.
...
10. At the Windows PowerShell prompt, enter the following command, and then press Enter:
...
1. On **LON-SVR1**, at the **Windows PowerShell** prompt, enter the following command, and then press Enter:

   ```
   Set-SmbServerConfiguration -AuditSmb1Access $true
   ```

2. Select **Yes** at the confirmation message.
