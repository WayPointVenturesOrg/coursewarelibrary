...
You work at a publishing house handling information about books and publishers. Your managers are impressed with your knowledge and SQL skills, and want to offer you a promotion. They want you to add an entry to their **employee** table with your information. You will use the `INSERT` statement to add a row with your data. Once you do so, you will use the `UPDATE` statement to change the information in that row to reflect your enhanced role.
...
You will begin by using Microsoft SQL Server Management Studio to connect to the publishing house's database, called **pubs**. This will allow you to add, retrieve, and change information within the database.
...
Now that you have successfully connected to the **pubs** database, you will inspect the **employee** table to determine what data types, constraints, and properties it has. This step will allow you to accurately predict what values it can accept.
...
2. Press the right arrow key to expand the database folder.

   > The database folder expands.

3. Inside the *pubs* database folder, locate the **Tables** folder and select it. Press the right arrow key to expand the folder.

   > The folder expands.

4. Inside the **Tables** folder, locate the *dbo.employee* table folder and select it. Press the right arrow key to expand the table folder.
...
5. Inside the *dbo.employee* table folder, locate and expand the *Columns* folder.
   ...
   > For example, the **minit** column is written `minit (char(1), null)`, which indicates the data type is *char(1)* and the column accepts NULL values.
...
8. Inside the *Constraints* folder, select and open the *CK_emp_id* entry.
   ...
   > The **MIA-SQL.pubs - dbo.employee** page opens and a popup window opens named **Check Constraints**.
   > In the **Check Constraints** window, information about this constraint is detailed.
   > Under the *General* dropdown, the *Expression* contains information about what values are accepted by this **Check** constraint.
...
2. Right-click or access the context menu for the *dbo.employee* table, and select **Design**.
   ...
   > Select a column name to view additional details in the **Column Properties** window.

3. Select the **emp_id** column. Inspect the **Column Properties** window.

   > This window contains additional information that may be difficult to find through the **Object Explorer**.
...
You can now use Microsoft SQL Server Management Studio to closely inspect a table in order to determine its features and the values it can accept.

## Exercise 3: Add and change data in a database using INSERT and UPDATE statements

### Scenario

Now that you have inspected the **employee** table, you will add a new entry with an `INSERT` statement that matches the table's data type requirements and constraints. Once you have added the entry, you will change it using an `UPDATE` statement.
...
1. In a new query to the **pubs** database, write the following `SELECT` statement in the **query** window:
...
3. Delete the previous statement and then write the following `INSERT` statement:
...
4. Alter the statement by replacing `John` with your first name, `Doe` with your last name, and `D` with the first letter of your middle name. If you don't have a middle name, you can replace it with an empty string by writing `''`.

   > If you don't want to use your real name, you can use a famous individual's name instead.

5. Select the **Execute** button, or press **`F5`** on the keyboard to run the statements.
...
6. Retrieve the resulting table by writing and executing the following `SELECT` statement:
...
1. In a new query to the **pubs** database, write the following `UPDATE` statement in the **query** window:
...
4. Delete the previous statement and write and execute the following `SELECT` statement:
