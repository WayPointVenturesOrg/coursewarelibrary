...
You want to make DNS zone management easier. You want to configure DNS policies in Windows Server, so that users in different geographical areas can connect to a different web server. You must then test and troubleshoot the DNS configuration that you have created.
...
If you configure the DNS policies correctly, when clients in the Toronto location query for the IP address of `www.contoso.com`, they should receive different name resolution answers than clients in the London location who are mapping to `www.contoso.com`.
...
4. Right-click **Contoso.com**, and then select **New Alias (CNAME)**.

5. In the **New Resource Record** window, in the **Alias Name** text box, type **www**, in the **Fully qualified domain name (FQDN) for target host** text box, type **LON-DC1.Contoso.com**, and then select **OK**. 
...
8. In the **Windows PowerShell** console, enter the following two commands, and press Enter after each command:
...
11. Right-click the **Start** icon and select **Windows Terminal (Admin)**.

12. In the **Administrator: Windows PowerShell** console, enter the following two commands, and press Enter after each command:
...
1. Switch to **LON-DC1**, right-click **Start**, and then select **Windows PowerShell**.

2. On **LON-DC1**, in the **Windows PowerShell** window, enter the following command, and then press Enter: 
...
3. At the Windows PowerShell command prompt, enter the following cmdlets, and press Enter after each cmdlet:
...
2. In the **Administrator: Command Prompt** window, type the following two commands, and press Enter after each command:
   ...
   > You should get the result **172.16.0.41** since LON-CL1 is in the UKZoneScope.
   >

3. Switch to **TOR-SVR1**.

4. In the **Windows PowerShell** window, type the cmdlets, and press Enter after each one: 
   ...
   > You should get a result of **172.16.18.17** since TOR-SVR1 is in the CanadaZoneScope.

**Results:** After completing this exercise, you will have configured DNS policies, and then tested that the policies work successfully.
...
5. On **LON-CL1**, select **Start**, type **network conn**, and then select **View network Connections**. 
...
7. In the **Network Connections** window, right-click the **PAC_WAN** connection to open the context menu, and then select **Enable**. 
   ...
   > **Note:** By doing this, you disconnect LON-CL1 from the London network and connect it to the PAC_WAN network.
...
11. In the **Preferred DNS server** box, type **172.16.19.20**, and then click **OK**.

12. In the **PAC_WAN Properties** dialog box, click **Close**.

### Task 2: Use DNS tools to confirm proper client configuration

1. On **LON-CL1**, in Windows PowerShell, enter the following cmdlets, and press Enter after each line:
...
5. If the address still shows as **172.16.0.50**, perform the following:
   ...
   - On **LON-CL1**, in the **Windows PowerShell** console, enter the following cmdlet, and then press Enter:

     `Register-DnsClient` 

   - Switch to **SYD-SVR1**. In the console tree, right-click **Contoso.com**, and then select **Refresh**.


   > The host record for **LON-CL1** should have an IP address of **172.16.19.50**.
   >

6. In the console tree, right-click **SYD-SVR1**, and then select **Clear Cache**.


### Task 3: Test DNS name resolution to external and internal hosts

1. On **LON-CL1**, in the **Windows PowerShell** console, enter the following cmdlets, and press Enter after each line:
...
**Results:** After completing this exercise, you should have validated the implementation of the DNS infrastructure.
...
| **Additional Information**: Susan is the only one  affected in the department. She cannot access the Research data folder on LON-DC1. |
| **Plan of Action**:                                          |
| **Resolution**:                                              |
...
3. In File Explorer, in the address bar, enter **\\\\LON-DC1\\Labfiles\\Mod04**, and then press **Enter.**

4. In the details pane, right-click **Scenario.vbs**, and then select **Copy**.

5. In the console tree of **File Explorer**, select the **Documents** folder, and then, in the empty space of the details pane, right-click and select **Paste**. 
6. Double-click **Scenario.vbs** to run the script.
7. Close File Explorer.

### Task 3: Resolve the problem

1. On **LON-CL1**, while still in **Windows PowerShell**, enter the following command, and then press Enter:
...
3. At the **Windows PowerShell** command prompt, enter the following command, and then press Enter. Notice that the address returned is the default gateway:

   ```
   test-connection lon-dc1
   ```

4. At the **Windows PowerShell** command prompt, enter the following command, and then press Enter. Notice that the wrong IP address is returned for **LON-DC1**:

   ```
   Get-DnsClientCache | fl
   ```

5. At the **Windows PowerShell** command prompt, enter the following command, and then press Enter. Notice that the correct record is returned from the Domain Name System (DNS) server:
...
7. Scroll to the end of the file, delete **172.16.0.1 lon-dc1.Contoso.com**, and then press Enter. 

   > Note: Be sure to verify that there are no other entries listed. If there are, delete the additional entries that refer to lon-dc1.
...
10. At the **Windows PowerShell** command prompt, enter the following command, and then press Enter:

    ```
    test-connection lon-dc1
    ```

11. At the **Windows PowerShell** command prompt, enter the following command, and then press Enter. You can now see the correct record for **LON-DC1** in the cache:

    ```
    Get-DnsClientCache | fl
    ```

12. At the **Windows PowerShell** command prompt, enter the following command, and then press Enter. Note that the command runs successfully:
...
14. In the **File Explorer** address bar, enter **\\\\LON-DC1\\Labfiles**, and then press Enter. The folder opens.
