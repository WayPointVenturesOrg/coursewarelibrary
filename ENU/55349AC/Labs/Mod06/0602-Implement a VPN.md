...
Contoso Corporation has decided to implement a virtual private network (VPN) solution. The solution will be based upon the SSTP protocol.
...
To implement a VPN solution, you need to first verify and configure certificate requirements for the SSTP protocol. You then must configure the Remote Access server to provide VPN connectivity, and create a remote access policy to ensure that the clients can connect to the server by using PPTP and SSTP.
...
12. In the **Certificate Properties** dialog box, on the **Subject** tab, under **Subject name**, under **Type**, select **Common name**.
...
5. In the **Add Site Binding** dialog box, under **Type**, select **https**, in the **SSL Certificate** list, select the **131.107.0.10** certificate, select **OK**, and then select **Close**.
...
2. Right-click **EU-RTR (local)**, and then select **Configure and Enable Routing and Remote Access**.
...
10. Expand **EU-RTR (local)**, right-click **Ports**, and then select **Properties**.
...
16. Right-click **EU-RTR (local)**, and then select **Properties**.
...
3. In the navigation pane, right-click **Network Policies**, and then select **New**.
...
Now that you have deployed the VPN solution, you need to verify that the clients can connect by using VPN. 

The main tasks for this exercise are as follows:

1. Validate that LON-CL1 is an external client.
2. Configure a VPN connection and verify connectivity.

### Task 1: Validate that LON-CL1 is an external client

1. Switch to **LON-CL1** and sign in as **Admin** with the password of **Pa55w.rd**.

2. Right-click **Start**, and then select **Network Connections**.
...
6. Right-click **Internet**, and then select **Properties**.
...
12. In File Explorer, in the address bar, enter **`\\LON-DC1\`**, and then press Enter. Notice that a Network Error message displays.
...
#### Create a VPN profile and connect using PPTP
...
6. In the **Sign in** dialog box, in the **User name** text box, enter **Contoso\Tonnie**, in the **Password** text box, enter **Pa55w.rd**, and then select **OK**.
...
1. In the **Network Connections** window, right-click **Contoso VPN**, and then select **Properties**.
...
8. In the **Network sign-in** dialog box, in the **User name** box, enter **Contoso\Tonnie**, in the **Password** box, enter **Pa55w.rd**, and then select **OK**.
...
Tonnie reports that she cannot connect to the Contoso VPN. You received an incident report, and you must investigate and fix the issue.
...
| **Incident  Details**                            | The Contoso VPN connection is suddenly not working on Tonnie’s computer, and she cannot access intranet resources from her home network. |
| **Additional Information**                       | Tonnie cannot connect to intranet resources from home. Tonnie must be able to connect by using VPN from her computer, LON-CL1. |
| **Plan  of Action**                              |                                                              |
| The main tasks for this exercise are as follows: |                                                              |
...
### Task 3: Simulate the issue and try to connect by using the Contoso VPN connection

1. On **LON-CL1**, sign in by using the user name **Admin** and the password **Pa55w.rd**.

2. Right-click **Start**, and then select **Windows Terminal (Admin)**. When you receive a prompt in **User Account Control** (UAC), select **Yes**.
...
4. At the command prompt, enter the following commands, and then press Enter after each one:
...
10. Wait for the connection attempt to fail, and then write down the error message in the **Plan of Action** section of the incident record in the Student Handbook. (If the connection is successful, disconnect and the re-attempt the connection. It should fail.)
...
2. In the **File Explorer** window, in the address bar, enter **`\\172.16.0.10\C$\`**, and then press Enter.

3. In the **Windows Security** dialog box, enter **Contoso\Administrator** in the **User name** text box, enter **Pa55w.rd** in the **Password** text box, and then press Enter.

4. In the **File Explorer** window, right-click **ContosoRootCA.cer**, select **Show more options**, and then select **Install Certificate**.
...
11. Right-click **Start**, and then select **Windows Terminal**.

12. In the **Command Prompt** window, enter **mmc**, and then press Enter.

13. If the **User Account Control** dialog box is displayed, select **Yes**.
...
20. In the **Sign-in** dialog box, in the **User name** text box, enter **Contoso\Tonnie**, in the **Password** text box, enter **Pa55w.rd**, and then select **OK**.
